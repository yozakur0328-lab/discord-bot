...
const CHANNEL_ID = "1486822569528004608";
...
async function sendSummary() {
  ...
  let text = `📋 ${today.getMonth() + 1}/${today.getDate()} 17:00 参加者リスト\n\n`;
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    const [, c, i] = interaction.customId.split("_");
    ...
    const newRows = interaction.message.components.map((row) => {
      ...
      const [, rc, ri] = btn.customId.split("_");
  ...
  if (interaction.isChatInputCommand()) {
    if (interaction.commandName === "list") {
      ...
      let text = `📋 ${today.getMonth() + 1}/${today.getDate()} 現在の参加者一覧\n\n`;
